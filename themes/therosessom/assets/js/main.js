...
// Import external libraries (Swiper 제외)
// Swiper는 동적으로만 import
import AOS from 'aos';
...
class TheRosessomTheme {
  ...
  /**
   * Initialize Swiper sliders with dynamic import
   */
  async initSwipers() {
    ...
    // If no sliders found, skip initialization
    if (!heroSlider && portfolioSliders.length === 0 && !testimonialSlider) {
      console.log('No slider elements found, skipping Swiper initialization');
      return;
    }

    try {
      // Dynamic import Swiper only when needed
      const [{ default: Swiper }] = await Promise.all([
        import('swiper/bundle'),
        import('swiper/css/bundle')
      ]);

      // Hero slider - Minimal version without navigation
      if (heroSlider) {
        const slideCount = heroSlider.querySelectorAll('.swiper-slide').length;
        
        // Only initialize if multiple slides
        if (slideCount > 1) {
